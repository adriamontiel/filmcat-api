...
import { fetchBizcochitoSessions } from './adapters/bizcochito.js'
import { REGISTRY_INDEX } from './registry.js'
...
async function resolveShowtimes(name, filmTitle, date) {
  ...
  // Sistema "bizcochito" (Admit One Spain)
  if (entry.system === 'bizcochito') {
    try {
      const sessions = await fetchBizcochitoSessions(entry.domain, filmTitle, date)
  ...
  // Sistema desconegut → tractam com no_source
  return { cinema: name, web: entry.web, status: 'no_source', stale: false, sessions: [] }
}
